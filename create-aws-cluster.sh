...
create_cluster() {
  local CLUSTER_NAME=${CLUSTER_NAME:-"kjoshi-insights-cluster-$(date +%Y%m%d-%H%M%S)"}
  dkp create bootstrap
  #kind get kubeconfig --name konvoy-capi-bootstrapper > kubeconfig-bootstrap

  # We have to load this and upload it to the Bootstrap Cluster due to ratelimiting issues.
  #docker pull docker.io/mesosphere/kommander2-runtime-extension:v0.3.0
  #kind load docker-image docker.io/mesosphere/kommander2-runtime-extension:v0.3.0 -n konvoy-capi-bootstrapper
  
  #dkp create bootstrap

  dkp create cluster aws \
    --ami=ami-099865d707086c842 \
    --self-managed=true \
